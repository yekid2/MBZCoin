...
def main():
    ...
    tmpdir = "%s/mbzcoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    if not (enable_wallet and enable_utils and enable_bitcoind):
        print("No functional tests to run. Wallet, utils, and mbzcoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, jobs=1, enable_coverage=False, args=[], combined_logs_len=0):
    ...
    try:
        if subprocess.check_output(["pidof", "mbzcoind"]) is not None:
            print("%sWARNING!%s There is already a mbzcoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    if "LITECOIND" not in os.environ:
        os.environ["LITECOIND"] = build_dir + '/src/mbzcoind' + exeext
        os.environ["LITECOINCLI"] = build_dir + '/src/mbzcoin-cli' + exeext
...
class RPCCoverage():
    ...
    commands per `mbzcoin-cli help` (`rpc_interface.txt`).
